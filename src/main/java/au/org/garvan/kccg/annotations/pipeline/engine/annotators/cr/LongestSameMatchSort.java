...
public class LongestSameMatchSort {
	...
	public LongestSameMatchSort(List<ConceptAnnotation> annotations) {
	...
	private void sort() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (int j = 0; j < list.size(); j++) {
				...
				if (candidateA.getStartOffset() >= candidateB.getStartOffset() && candidateA.getEndOffset() <= candidateB.getEndOffset()) {
					if (candidateA.getConcept().getUri().equalsIgnoreCase(candidateB.getConcept().getUri())) {
						found = true;
						break;
					}
				}
