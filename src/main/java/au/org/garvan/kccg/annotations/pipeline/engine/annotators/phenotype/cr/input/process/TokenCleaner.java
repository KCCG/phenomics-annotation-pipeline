...
public class TokenCleaner {
    ...
    private void processSplit(String token, String shape, String pos) {
        ...
        if (StringUtils.isNumeric(cleanToken)) {
            ...
            //CR:DONE This is a workaround, currently pos is not updated based on dictionary, will solve it when Index is included. Commenting
            pos = PseudoTA.getCrResources().getSimpleDictionary(TAConstants.DICT_POS).hasKey(cleanToken) ?
                    PseudoTA.getCrResources().getSimpleDictionary(TAConstants.DICT_POS).getValue(cleanToken) :
                    pos;
    ...
    private String clean(String token, boolean containsX) {
        ...
        //CR:DONE This is a workaround, currently token is not updated based on dictionary, will solve it when Index is included. Commenting
        if (!containsX) {
            cleanToken = PseudoTA.getCrResources().getSimpleDictionary(TAConstants.DICT_ORDINALS).hasKey(cleanToken) ?
                    PseudoTA.getCrResources().getSimpleDictionary(TAConstants.DICT_ORDINALS).getValue(cleanToken) :
                    cleanToken;
        }
        return cleanToken;
    }
